...
const WrappedGToken = artifacts.require('WrappedGToken')
const MockWETHGateway = artifacts.require('MockWETHGateway')

contract('WrappedGToken', (accounts) => {
  ...
  before(async () => {
    ...
    wToken = await WrappedGToken.new(
      'Wrapped',
      'W',
      wethGateway.address,
      wethGateway.address, // Mock gateway acts as the lending pool
      wethGateway.address,
      feeToSetter,
    )
  })
  ...
  describe('#wrap', () => {
    it('should deposit correctly given a debtAmount', async () => {
      ...
      const balanceBefore1 = toBN(await web3.eth.getBalance(sender))
      const wBalanceBefore1 = await wToken.balanceOf(sender)
      await wToken.wrap({ value: underlyingAmount, gasPrice: 0 })
      const balanceAfter1 = toBN(await web3.eth.getBalance(sender))
    ...
    it('should work', async () => {
      ...
      const balanceBefore1 = toBN(await web3.eth.getBalance(sender))
      const wBalanceBefore1 = await wToken.balanceOf(sender)
      await wToken.wrap({ value: amount1, gasPrice: 0 })
      const balanceAfter1 = toBN(await web3.eth.getBalance(sender))
      const wBalanceAfter1 = await wToken.balanceOf(sender)
      ...
      const balanceBefore2 = toBN(await web3.eth.getBalance(sender))
      const wBalanceBefore2 = await wToken.balanceOf(sender)
      await wToken.wrap({ value: amount2, gasPrice: 0 })
      const balanceAfter2 = toBN(await web3.eth.getBalance(sender))
  ...
  describe('interest bearing', () => {
    it('reflect interest earned in the underlying', async () => {
      ...
      await wethGateway.depositETH(wethGateway.address, wToken.address, 0, {
        value: 1500,
      })
  ...
  describe('#unwrap', () => {
    it('should work', async () => {
      ...
      const balanceBefore1 = toBN(await web3.eth.getBalance(sender))
      const wBalanceBefore1 = await wToken.balanceOf(sender)
      await wToken.unwrap(amount1, { gasPrice: 0 })
      const balanceAfter1 = toBN(await web3.eth.getBalance(sender))
      const wBalanceAfter1 = await wToken.balanceOf(sender)
      ...
      const balanceBefore2 = toBN(await web3.eth.getBalance(sender))
      const wBalanceBefore2 = await wToken.balanceOf(sender)
      await wToken.unwrap(amount2, { gasPrice: 0 })
      const balanceAfter2 = toBN(await web3.eth.getBalance(sender))
      const wBalanceAfter2 = await wToken.balanceOf(sender)
  ...
  describe('#takeFee', () => {
    it('should work', async () => {
      let feeToBalanceBefore = toBN(await web3.eth.getBalance(feeToSetter))
      await wToken.takeFee()
      let feeToBalanceAfter = toBN(await web3.eth.getBalance(feeToSetter))
      feeToBalanceAfter.should.be.eq.BN(feeToBalanceBefore.add(toBN(150)))

      // disallow double claim
      feeToBalanceBefore = toBN(await web3.eth.getBalance(feeToSetter))
      await wToken.takeFee()
      feeToBalanceAfter = toBN(await web3.eth.getBalance(feeToSetter))
      feeToBalanceAfter.should.be.eq.BN(feeToBalanceBefore)
    })
  })
})
